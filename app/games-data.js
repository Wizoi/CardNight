...
const GAMES = [
  ...
  },
  {
    ...
    players: { min: 5, max: 8 },
    before: [],
    betting: `Ante 50¢, 25¢ raise increment, $2 max bet per person (house default). A round right after the deal, then another after each of the 5 community-card flips.`,
    setup: `Deal 3 cards face down to each player.`,
    gameplay: `A betting round right after the deal. Then the dealer flips 1 community card per round, 5 rounds total, with another betting round after each. Anyone holding a card matching the flipped rank must discard it. Aces count as 1 or 11, face cards count as 10. If a flipped card's rank already came up in an earlier round, it's set aside and a new card is drawn in its place.`,
    win: `Best high hand AND best low hand split the pot — or a player who discards their entire hand wins it outright. Being dealt AAA ("Ultima") wins the whole pot immediately, no flips or betting needed.`,
    keyDecisions: [
      "How aggressively to bet — your own hand only shrinks as ranks get discarded, never improves with new cards",
    ],
    repeats: [
      "Three aces off the deal is an instant win, skipping betting entirely",
    ...
    script: `You get 3 cards, then we bet. I'll flip one community card at a time, 5 flips total, with a betting round after each — if you're holding that rank, discard it. If the same rank comes up twice, I'll swap it for a new card instead of flipping it again. Aces count 1 or 11, face cards count 10. Best high hand and best low hand split the pot, or whoever empties their hand first wins it all. Three aces off the deal wins on the spot, no betting needed.`,
  },
  {
    id: "3-5-7-guts",
    name: "3-5-7 Guts",
    ...
    isNew: false,
    icon: "🔥",
    players: { min: 5, max: 8, note: "7 cards a hand (6 at a full 8-player table), same deck-size scaling as Deep or Double Screw." },
    before: [],
    betting: `Ante only — 50¢ house default. No raises, no max bet. Escalating pot: every non-winning stayer matches it to keep the cycle going.`,
    setup: `Deal 7 cards to each player (6 at a full table) all at once — not in escalating rounds. Pass some cards left, some right (2 left / 1 right at 7 cards, 1 left / 1 right at 6), same as Deep or Double Screw.`,
    gameplay: `3s, 5s, and 7s are ALL wild together for the whole hand, known from the start — not escalating round to round. Decide in or out; among those in, best hand wins the pot outright, and it just keeps escalating (every other stayer matches it) until exactly one player stays in and wins solo.`,
    win: `Last player standing, whenever that happens — same open-ended cycle as Deep or Double Screw / 3 Buy 5, not a fixed round count.`,
    keyDecisions: [
      "Which cards to pass left/right before your hand is set",
      "Fold or stay in, each escalating round, against a pot that only ever grows",
    ],
    repeats: [
      "3s, 5s, and 7s are wild from the very first look at your cards — the wild rank never changes mid-hand",
      "The cycle keeps escalating until exactly one player wins outright — there's no fixed number of rounds",
    ],
    script: `You get 7 cards, passed and set just like Deep or Double Screw. 3s, 5s, and 7s are all wild together the whole hand — no escalating wild rank to track. In or out; best hand among those in takes the pot, and if more than one of you stayed in, everyone who didn't win has to match it and we go again. Keeps escalating till exactly one of you stays in and wins outright.`,
  },
  {
    id: "5-5-21",
    name: "5.5-21",
    ...
    players: { min: 5, max: 8 },
    before: [],
    betting: `Ante 50¢, 25¢ raise increment, $2 max bet per person (house default). A round right after the first card is dealt, then another after each full round where everyone gets to take a card or stand.`,
    setup: `Deal 1 card face down to each player.`,
    gameplay: `A betting round right after the deal. Then each round, take another card or pass, followed by another betting round. Face cards are worth 0.5, Aces are worth 1. You CANNOT go over either target — going over busts you out of that half. Once a full round passes with nobody taking a card, one more complete no-taker round is required before dealing ends.`,
    ...
    keyDecisions: [
      ...
      "How aggressively to bet — fold once you're busted on both sides, since you've got nothing left to play for",
    ],
    repeats: [
      "Going over busts you here — opposite of 7-27",
      "Ties go to whoever's holding fewer cards, not a split",
      "Busted on both sides? Fold — there's no hand left to win with",
    ],
    script: `You start with 1 card face down, then we bet. Each round, take another or pass, then we bet again — face cards are half a point, aces count 1. Go for the low target (5.5) or the high one (21), but go over either and you're busted out of that half. If you're busted on both, fold — nothing left to chase. Once everybody passes for a full round, one more no-taker round and we're done — closest to 5.5 and closest to 21 split the pot.`,
  },
  {
    id: "7-27",
    name: "7-27",
    ...
    betting: `Ante 25¢ — half the usual 50¢ house default. 25¢ raise increment, $2 max bet per person otherwise standard. A round right after the initial deal (down + up), then another after each round where everyone gets to take another card or stand — ending after just ONE round of nobody taking a card (not two, unlike 5.5-21). You can fold anytime, even though nobody can bust here.`,
    setup: `Deal 1 card down, then 1 card up to each player.`,
    gameplay: `A betting round right after the deal. Then each round, optionally take another card, followed by another betting round. Face cards are worth 0.5, Aces are worth 1 or 11 free of charge. A 10 counts as 10 by default — pay $1 and it becomes flexible, worth 0 or 10, your choice. Players may buy back ("down the river") their up-card to hide it again — escalating cost, up to 3 total: $1, then $2, then $3 later — "use it or lose it," must decide the moment it's offered. Going over is allowed and doesn't bust you.`,
    win: `Closest to 7 wins the low half of the pot, closest to 27 wins the high half — regardless of how many cards a player holds. Ties split evenly. If Kitchen Sink is on: a hand reading exactly 7 for low and exactly 27 for high at the same time can't be beaten or tied out of either half — it wins the entire pot outright instead of splitting it. Unlike 3-33's "Ultima" (an instant win off the deal alone), a Kitchen Sink is normally something you build toward during the hand — hitting, and paying for buy-backs and the flexible-10 option to land on the exact combination — and it's only checked at showdown, not at deal time. More than one Kitchen Sink at the table splits the pot between them.`,
    keyDecisions: [
      ...
      "How aggressively to bet — you can fold anytime, nobody busts here but a hopeless hand still isn't worth chasing",
    ],
    repeats: [
      ...
      "Just ONE round of nobody taking a card ends the hand here — 5.5-21 needs two",
    ],
    script: `One card down, one up, then we bet. You can buy your up-card back to hide it again — a dollar the first time, two the second, three later for a third, and you have to decide right when it's offered. Each round you can take another card or stand, then we bet again — one round where nobody takes a card and we're done. Face cards are half a point, aces are 1 or 11 for free — but a ten's stuck at 10 unless you pay a dollar to make it 0-or-10 flexible. Going over doesn't bust you here, but you can still fold if your hand's hopeless. Closest to 7 and closest to 27 split the pot — unless we're playing Kitchen Sink, where a hand that's exactly 7 and exactly 27 at the same time wins the whole thing outright.`,
  },
  {
    id: "acey-ducey",
    name: "Acey Ducey",
