...
- **Ending:** After each round's decision, hands are revealed among the players who stayed in, and the lowest hand must match the pot and pay the highest hand. The game ends when the pot is awarded to the last remaining player after round 3. The source also claims the game "concludes early if a player wins five consecutive rounds," but since a single hand only has 3 rounds, that can't happen within one hand — the source never explains whether/how a new hand re-deals after round 3, so treat this early-win rule as unconfirmed/unclear rather than a settled mechanic until we've actually played it out.
