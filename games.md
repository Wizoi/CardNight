...
- **Wildcards:** 3s, 5s, AND 7s are all wild together, for the whole hand, known from the very first look at your cards — not escalating round to round. Optional dealer's-choice: add 1-2 additional flip-up wildcard(s) on top, same as Deep or Double Screw — revealed only **after passing**, so the flip-up rank never factors into what anyone chooses to pass away.
...
- **Optional rule — Kitchen Sink:** Dealer's choice whether this is in play. A hand that reads as exactly 7 for low AND exactly 27 for high at the same time (using the flexible ace value and/or a paid-flexible 10) is a "Kitchen Sink" — the best possible hand on both sides at once, since it can't be beaten or tied out of either half. Unlike 3-33's "Ultima" (an instant win off the initial deal alone, no play involved), a Kitchen Sink is normally something a player builds toward DURING the hand — hitting, and paying for buy-backs and the flexible-10 option to reshape a hand into the exact combination — and it's only checked at showdown, after all betting and hitting is done, not at deal time. If this rule is on, a Kitchen Sink wins the entire pot outright instead of splitting it. If more than one player has one, they split the pot evenly between themselves, per the usual tie rule.
...
- **Wildcards:** Optional, dealer's choice — the center card's rank is wild, everywhere it shows up (in anyone's hand, or the center card itself, which is already part of the cross — not a separate flip-up addition).
...
- **Winning:** Among players who declared "in," best 5-card poker hand (using the wild ranks/Joker revealed so far) wins the whole pot outright. Anyone who declared "in" and lost must match the ante pool — everyone's ante added together (e.g. $3.50 at 7 players and the standard 50¢ ante), not the whole pot (which also includes everyone's row-betting contributions) — to play the next hand; the pot still escalates hand over hand the same way every other Guts game's cycle does, ending only once a single player is "in" alone and wins uncontested.
